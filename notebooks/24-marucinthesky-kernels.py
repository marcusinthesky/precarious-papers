...
try:
    ...
    centrality = pd.Series(nx.eigenvector_centrality(paradise_graph, tol=1e-7, max_iter=5000)).to_frame('Centrality')
...
D = (
    ...
    .replace(0, np.nan)
    .pipe(lambda df: df.apply(stats.weibull_min(*stats.weibull_min.fit(df.melt().value.dropna(), floc=0, f0=1)).pdf))#stats.geom(prob).pmf))#
    .fillna(0))
graph = nx.from_pandas_adjacency(D)
# graph_centrality = pd.Series(nx.eigenvector_centrality(graph), name='Matched_Centrality')

# %%
dist = (renamed_distances.loc[features.index, features.index]
    .replace(0, np.nan)
    .pipe(lambda df: stats.weibull_min.fit(df.melt().value.dropna(), floc=0,  f0=1.)))
dist
...
# %%
merged_centralities = matched.loc[:, ['node_id', 'symbol']].merge(centralty, left_on='node_id', right_index=True, how='inner').groupby('symbol').Centrality.mean()
graph_centrality = pd.Series(nx.eigenvector_centrality(graph), name='Matched_Centrality')
features_centrality = features.join(merged_centralities.rename('ICIJ_Centrality'), how='left')

# %%
from scipy.stats.mstats import winsorize

# %%
X, y = (features_centrality
        .assign(ri_rf = lambda df: winsorize(df.returns, [0.028 - 0.0, 0.062 + 0.02]), #[0.028, 0.062]
                rm_rf = lambda df: df.rm).drop(columns=['returns', 'rm'])
...
# %%
stats.kstest(features_centrality.returns, stats.norm(features_centrality.returns.mean(), features_centrality.returns.std()).cdf)

# %%

# %%
y.hvplot.hist(title='Histogram of Winsorized Excess Returns', height=400, wight=800) *\
pd.Series(np.random.normal(y.mean(), y.std(), 10000)).hvplot.kde(fill_alpha=0, xlabel='Excess Returns', label='Samples from Moment Matching Normal Distribution')

# %%

# %%
ols = OLS(y, X.assign(alpha=1).drop(columns = ['Price_to_Earnings', 'Market_Capitalization', 'ICIJ_Centrality'])).fit()
ols.summary()
...
pd.Series(outlier_results.cooks_distance[0], y.index).sort_values(ascending=False).hvplot.bar(width=1200, title='Cooks Distance').opts(xrotation=90)
...
# %%
pd.Series(H, y.index).sort_values(ascending=False).hvplot.bar(width=1200, title='Leave-one-out pairwise standardized dfbeta distances').opts(xrotation=90)
...
# %%
degree = np.mean(np.array(list(dict(paradise_graph.degree()).values()), dtype='float64'))
...
huber = Pipeline([('scaler', StandardScaler()), ('model', HuberRegressor(epsilon=2.25, alpha=0.))])#2.028
huber.fit(X.assign(Wy = W@y), y)
huber_outliers = huber.named_steps['model'].outliers_
print('# Outliers: ', huber_outliers.sum())

# %%
huber_outliers_set = set(y.loc[huber_outliers, :].index.tolist())# | {'DNN', 'FGEN', 'EXEL', 'SMMF', 'MASI', 'TAT', 'CL', 'GLBS', 'THO', 'SHIP', 'HNW'}
huber_outliers_set
...
component = (hvnx.draw(graph, pos, edge_color='weight', edge_cmap='greys',
          edge_width=hv.dim('weight')*25, node_size=hv.dim('size')*20
                      ).opts(title='Cooks Distance', colorbar=True) *\
...
# %%
hvnx.draw_kamada_kawai(graph, node_cmap='turbo',
                         node_size=150,
...
spatial_ols = ps.model.spreg.OLS(y.to_numpy(), Z.to_numpy(), w=w, moran=True, spat_diag=True, white_test=True, robust=None,
...
pd.DataFrame(vec.real, columns=['Component 1', 'Component 2'], index=W.index).assign(ticker = W.index).hvplot.scatter('Component 1', 'Component 2', color='ticker')
...
# %%
J = pd.DataFrame(V, index=W.index, columns=W.index).apply(lambda df: df / df.sum(), axis=1)
...
P = pd.DataFrame(pca.fit_transform(F), index=X.index).rename(columns=lambda s: 'Component ' + str(s+1))
...
# %%
first, second = map(lambda s: '(' + str(s) + '%)', np.round(pca.named_steps['pca'].explained_variance_ratio_[:2]*100, 2))

# %%
pd.DataFrame(pca_object.components_, index=F.columns.str.replace('_', ' ')).rename(columns=lambda s: 'Component ' + str(s+1)).reset_index().hvplot.labels(x='Component 1',
...
spatial_slx = ps.model.spreg.OLS(y.to_numpy(), Z.to_numpy(), w=w, moran=True, spat_diag=True, white_test=True, robust=None,
                                 name_w='Weibull Kernel over Shortest Path Length', name_ds='Paradise Papers Metadata', name_y=y.columns.tolist(), name_x=Z.columns.tolist())
...
# %%
U = pd.DataFrame(Pipeline([('scale', StandardScaler()), ('pca', PCA(2))]).fit_transform(Z), index=Z.index)
...
pd.Series(slx_outlier_results.cooks_distance[0], y.index).sort_values(ascending=False).hvplot.bar(width=1200, title='Cooks Distance SLX Model').opts(xrotation=90)
...
# %%
U = pd.DataFrame(outlier_pca.fit_transform(Z)[:, :2], index=Z.index).rename(columns=lambda s: 'Component ' + str(s+1))
top_cooks = pd.Series(outlier_results.cooks_distance[0], index=y.index).nlargest(3)

# %%
first, seconds = map(lambda s: '(' + str(s) + '%)', np.round(outlier_pca.named_steps['pca'].explained_variance_ratio_[:2]*100, 2))

# %%
U_cooks = U.assign(**{'Cooks Distance': outlier_results.cooks_distance[0]})

U_cooks.hvplot.scatter(x='Component 1', y='Component 2', 
                       xlabel='Component 1 '+first, ylabel='Component 2 '+ second, 
                       color='Cooks Distance',
                       cmap='turbo', 
                       logz=False,
                       width=900, height=500, 
                       title='Principal Components of our SLX Model with Cooks Outliers') *\
U_cooks.loc[top_cooks.index, :].reset_index().hvplot.labels(x='Component 1', y='Component 2', text='symbol', text_baseline='bottom',)

# %%
pd.Series(outlier_pca.named_steps['pca'].explained_variance_ratio_).hvplot.bar(title='Variance Explained Ratio of Principal Components of SLX Explanatory Variables')
...
# %%
U_cooks.assign(Clusters = clusters.astype(str)).hvplot.scatter(x='Component 1', y='Component 2', 
                       xlabel='Component 1 '+first, ylabel='Component 2 '+ second, 
                       color='Clusters',
#                        cmap='turbo', 
                       logz=False,
...
# %%
pearson_corr = pd.DataFrame(pairwise_distances((W@X).T, metric=lambda x, y: stats.pearsonr(x, y)[0]), index=X.columns, columns=X.columns).round(4).astype(str)
pearson_p = pd.DataFrame(pairwise_distances((W@X).T, metric=lambda x, y: stats.pearsonr(x, y)[1]), index=X.columns, columns=X.columns).round(4).astype(str)
...
pd.Series([variance_inflation_factor(X.values, i) 
              ...
              index=Z.columns, name='VIF').round(4).to_clipboard()
...
w = ps.lib.weights.full2W(W.to_numpy())
# Z = X.drop(columns=['Price_to_Earnings', 'Market_Capitalization', 'ICIJ_Centrality']).pipe(lambda df: df.join((W@df).rename(columns=lambda s: 'W_'+s))).drop(columns=[ 'W_rm_rf'])
spatial_ols = ps.model.spreg.ML_Error(y.to_numpy(), Z.to_numpy(), w=w, spat_diag=True, 
                                 name_w='Weibull Kernel over Shortest Path Length', name_ds='Paradise Papers Metadata', name_y=y.columns.tolist(), name_x=Z.columns.tolist())
print(spatial_ols.summary)
...
spatial_lag = ps.model.spreg.GM_Lag(y=y.to_numpy(), x=Z.to_numpy(), w=w, spat_diag=True, w_lags=2, robust='white',
                                 name_w='Weibull Kernel over Shortest Path Length', name_ds='Paradise Papers Metadata', name_y=y.columns.tolist()[0], name_x=Z.columns.tolist())
print(spatial_lag.summary)

# %%
# Z = X.drop(columns=['Price_to_Earnings', 'Market_Capitalization', 'ICIJ_Centrality'])
spatial_error = ps.model.spreg.ML_Error(y=y.to_numpy(), x=Z.to_numpy(), w=w, spat_diag=True, epsilon=1e-10,
                                 name_w='Weibull Kernel over Shortest Path Length', name_ds='Paradise Papers Metadata', name_y=y.columns.tolist()[0], name_x=Z.columns.tolist())
print(spatial_error.summary)

# %%
# Z = X.drop(columns=['Price_to_Earnings', 'Market_Capitalization', 'ICIJ_Centrality'])
spatial_error = ps.model.spreg.GM_Error(y=y.to_numpy(), x=Z.to_numpy(), w=w, 
                                 name_w='Weibull Kernel over Shortest Path Length', name_ds='Paradise Papers Metadata', name_y=y.columns.tolist()[0], name_x=Z.columns.tolist())
print(spatial_error.summary)

# %%
# Z = X.drop(columns=['Price_to_Earnings', 'Market_Capitalization', 'ICIJ_Centrality'])
spatial_error = ps.model.spreg.GM_Error_Het(y=y.to_numpy(), x=Z.to_numpy(), w=w, max_iter=10, step1c=True,
                                 name_w='Weibull Kernel over Shortest Path Length', name_ds='Paradise Papers Metadata', name_y=y.columns.tolist()[0], name_x=Z.columns.tolist())
print(spatial_error.summary)

# %%
# Z = X.drop(columns=['Price_to_Earnings', 'Market_Capitalization', 'ICIJ_Centrality'])
spatial_error = ps.model.spreg.GM_Combo(y=y.to_numpy(), x=Z.to_numpy(), w=w, w_lags=2, 
                                 name_w='Weibull Kernel over Shortest Path Length', name_ds='Paradise Papers Metadata', name_y=y.columns.tolist()[0], name_x=Z.columns.tolist())
print(spatial_error.summary)

# %%
# Z = X.drop(columns=['Price_to_Earnings', 'Market_Capitalization', 'ICIJ_Centrality'])
spatial_error = ps.model.spreg.GM_Combo_Het(y=y.to_numpy(), x=Z.to_numpy(), w=w, w_lags=2, step1c=False, inv_method="power_exp", max_iter=1000, epsilon=1e-10,
                                 name_w='Weibull Kernel over Shortest Path Length', name_ds='Paradise Papers Metadata', name_y=y.columns.tolist()[0], name_x=Z.columns.tolist())
print(spatial_error.summary)
...
# %%
Z = X.drop(columns=['Price_to_Earnings', 'Market_Capitalization', 'ICIJ_Centrality'])
spatial_error = ps.model.spreg.GM_Combo_Hom(y=y.to_numpy(), x=Z.to_numpy(), w=w, w_lags=2, max_iter=100,
                                 name_w='Weibull Kernel over Shortest Path Length', name_ds='Paradise Papers Metadata', name_y=y.columns.tolist()[0], name_x=Z.columns.tolist())
print(spatial_error.summary)
